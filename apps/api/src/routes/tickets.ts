...
export async function ticketRoutes(rawApp: FastifyInstance) {
  ...
  app.post("/api/webhooks/gitlab", {
    schema: {
      ...
      description:
        "Inbound webhook endpoint for GitLab issue and MR events. Secured via a token in X-Gitlab-Token header.",
    ...
    handler: async (req, reply) => {
      ...
      if (!webhookSecret) {
        logger.error("GITLAB_WEBHOOK_SECRET is not set — rejecting webhook request");
        return reply.status(401).send({ error: "Webhook secret not configured" });
      }

      const token = req.headers["x-gitlab-token"] as string | undefined;
      if (!token || token !== webhookSecret) {
        logger.warn("Webhook request missing or invalid X-Gitlab-Token header");
      ...
      if (event === "Issue Hook" && payload.object_kind === "issue") {
        const labels = payload.labels as Array<{ title: string }> | undefined;
        if (labels?.some((l) => l.title === "optio")) {
          logger.info({ issue: payload.object_attributes?.iid }, "GitLab issue labeled with optio");
      ...
      if (event === "Merge Request Hook" && payload.object_kind === "merge_request") {
        if (payload.object_attributes?.state === "merged") {
          const prUrl = String(payload.object_attributes?.url ?? "");
          const allTasks = await taskService.listTasks({ limit: 500 });
          const matchingTask = allTasks.find((t) => t.prUrl === prUrl);

          if (matchingTask) {
            logger.info({ taskId: matchingTask.id, prUrl }, "GitLab MR merged — resolving task");
            try {
              ...
              logger.info({ taskId: matchingTask.id, prUrl }, "Task completed via MR merge");
            } catch {
